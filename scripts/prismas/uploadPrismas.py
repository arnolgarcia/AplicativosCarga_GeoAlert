...
def uploadPrisma(tblprisma, connstring, StrEsquema, StrTabla):
    try:
        ...
        try:
            nroPrismas = tblprisma.elementos
            for prism in range(nroPrismas):
                myprisma = tblprisma.dataPrismas[prism]
                prismaName = myprisma.name
                prismaN = myprisma.elementos
                # Obtener idPrisma desde la BD, si el prisma no existe, se crea un nuevo id
                sqlGetid = "select  prismas.get_idloadprisma('%s')" % (str(prismaName))
                resultSet = conn.ExecuteSQL(sqlGetid)
                row = resultSet.next()
                prismaId = row.GetField(0)
